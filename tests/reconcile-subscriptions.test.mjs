...
// De instructeurtelling (prijsbepaling: Premium bevat 5, daarboven €35 p/m)
...
test('S1: 7 instructeurs op een €54,45-subscription → bijgewerkt naar €136,73', async () => {
  ...
  assert.deepEqual(mollie.calls.update[0].amount, { currency: 'EUR', value: '139.15' });
  assert.equal(mollie.calls.update[0].id, 'sub_S');
  assert.equal(mollie.calls.update[0].customerId, 'cst_S');

  // price_per_month = NETTO totaal (€45 + 2 × €35), niet de kale planprijs
  const upd = currentClient.calls[2].ops.find(([m]) => m === 'update');
  assert.equal(upd[1].price_per_month, 115);
  ...
  assert.equal(ev.payload.amount_to, '139.15');
});
...
test('S3: gekrompen team → bedrag gaat op dezelfde manier omlaag', async () => {
  resetSpies();
  mollie = makeMollie({
    getResult: { id: 'sub_S', status: 'active', amount: { currency: 'EUR', value: '139.15' } },
  });
  currentClient = makeClient(
    [
      { data: [], error: null },
